...
public class FileUtils {
    ...
    public static void appendTimingCSV(String path, int n, int p, long localNs, long sharedNs) {
        ensureParentDir(path);
        boolean fileExists = new File(path).exists();
        try (PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(path, true)))) {
            if (!fileExists) {
                pw.println("n,p,local_ms,shared_ms");
            }
            pw.printf(Locale.US, "%d,%d,%.2f,%.2f%n", n, p, localNs / 1_000_000.0, sharedNs / 1_000_000.0);
        } catch (IOException e) {
